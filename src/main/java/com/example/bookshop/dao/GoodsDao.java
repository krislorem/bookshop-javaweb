...
public class GoodsDao {
    ...
    //获取推荐栏商品列表
    public List<Map<String, Object>> getScrollGood() throws Exception {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
    ...
    //根据商品类型id查找
    public List<Goods> selectGoodsByTypeID(int typeID, int pageNumber, int pageSize) throws Exception {
        if (typeID == 0) {
    ...
    //计算该类型商品数
    public int getCountOfGoodsByTypeID(int typeID) throws Exception {
        String sql;
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        if (typeID == 0) {
    ...
    //查看推荐类型商品详情
    public List<Goods> selectGoodsbyRecommend(int type, int pageNumber, int pageSize) throws Exception {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        if (type == 0) {
            //当不添加推荐类型限制的时候
...
//        String sql = " select g.id,g.name,g.cover,g.image1,g.image2,g.intro,g.price,g.stock,t.name typename from goods g,recommend r,type t where g.id=r.goods_id and g.type_id=t.id and r.type=? order by g.id limit ?,?";
    ...
    //获取该推荐类型商品总数
    public int getRecommendCountOfGoodsByTypeID(int type) throws Exception {
...
//        String sql = "select g.id,g.name,g.cover,g.image1,g.image2,g.price,g.intro,g.stock,t.id typeid,t.name typename from goods g,type t where g.id = ? and g.type_id=t.id";
    ...
    //模糊搜索商品List
    public List<Goods> selectSearchGoods(String keyword, int pageNumber, int pageSize) throws Exception {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
//        String sql = "select * from goods where name like ? limit ?,?";
    ...
    //是属于推荐栏(轮播)商品
    public boolean isScroll(Goods g) throws Exception {
        return isRecommend(g, 1);
    }

    //是属于热销商品
    public boolean isHot(Goods g) throws Exception {
        return isRecommend(g, 2);
    }

    //是属于新品商品
    public boolean isNew(Goods g) throws Exception {
        return isRecommend(g, 3);
    }

    //判断该商品的类型(推荐、热销、新品)
    private boolean isRecommend(Goods g, int type) throws Exception {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
    ...
    //将该商品插入(推荐、热销、新品)类型
    public void addRecommend(int id, int type) throws Exception {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
    ...
    //删除该商品的类型(推荐、热销、新品)
    public void removeRecommend(int id, int type) throws Exception {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
