            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav">
                    <li><a href="${pageContext.request.contextPath }/index"
                           <c:if test="${param.flag==1}">class="active"</c:if>>图书商城</a></li>
                    <li class="dropdown">
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        ...
                                        <ul class="multi-column-dropdown">
                                            ...
                                            <li><a class="list" href="${pageContext.request.contextPath }/goods_list">全部系列</a>
                                            </li>

                                            <c:forEach items="${typeList}" var="t">
                                                <li><a class="list"
                                                       href="${pageContext.request.contextPath }/goods_list?typeid=${t.id}">${t.name}</a>
                                                </li>
                                            </c:forEach>
                    ...
                    <li><a href="${pageContext.request.contextPath }/goodsrecommend_list?type=2"
                           <c:if test="${param.flag==3 && t==2}">class="active"</c:if>>热销</a></li>
                    <li><a href="${pageContext.request.contextPath }/goodsrecommend_list?type=3"
                           <c:if test="${param.flag==3 && t==3}">class="active"</c:if>>新品</a></li>

                    <c:choose><%--switch--%>
                        <c:when test="${empty user }"><%--case:--%>
                            <li><a href="${pageContext.request.contextPath }/user_register.jsp"
                                   <c:if test="${param.flag==10 }">class="active"</c:if>>注册</a></li>
                            <li><a href="${pageContext.request.contextPath }/user_login.jsp"
                                   <c:if test="${param.flag==9 }">class="active"</c:if>>登录</a></li>
                        </c:when>
                        <c:otherwise><%--default:--%>
                            <li><a href="${pageContext.request.contextPath }/order_list"
                                   <c:if test="${param.flag==5 }">class="active"</c:if>>我的订单</a></li>
                            <li><a href="${pageContext.request.contextPath }/user_center.jsp"
                    ...
                    <c:if test="${!empty user && user.isadmin }">
                        <li><a href="${pageContext.request.contextPath }/admin/index.jsp" target="_blank">后台管理</a>
                        </li>
                    </c:if>
                </ul>
                <!--/.navbar-collapse-->
        ...
        <div class="header-info">
            <div class="header-right search-box" style="width: 225px">
                ...
                <form class="d-flex" role="search" action="${pageContext.request.contextPath }/goods_search">
                    <input class="form-control me-2" type="search" name="keyword" placeholder="书名/作者"
                           aria-label="Search">
            ...
            <div class="header-right cart">
                <a href="${pageContext.request.contextPath }/goods_cart.jsp">
                    <span class="glyphicon glyphicon-shopping-cart <c:if test="${param.flag==8 }">active</c:if>"
                          aria-hidden="true"><span class="card_num"><c:choose><c:when
                            test="${empty order}"></c:when><c:otherwise>${order.amount}</c:otherwise></c:choose></span></span>
